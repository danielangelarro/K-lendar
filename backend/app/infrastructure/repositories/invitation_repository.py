...
class InvitationRepository(IInvitationRepository):
    async def invite_users(self, event_id: uuid.UUID, user_ids: List[uuid.UUID]) -> None:
        ...
        for user_id in user_ids:
            user_event = UserEvent(user_id=user_id, event_id=event_id, status=EventStatus.PENDING)
            db.add(user_event)
        await db.commit()

    async def accept_invitation(self, event_id: uuid.UUID, user_id: uuid.UUID) -> None:
        db = await self.get_db()
        user_event = await db.execute(select(UserEvent).where(UserEvent.event_id == event_id, UserEvent.user_id == user_id))
